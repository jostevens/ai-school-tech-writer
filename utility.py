...
def format_data_for_openai(diffs, readme_content, commit_messages):
    ...
    prompt = (
        ...
        f"{readme_content}\n"
        "Consider the code changes and commit messages, determine if the README needs to be updated. If so, edit the README, ensuring to maintain its existing style and clarity. Ignore all readme changes from this project to update the readme\n"
        "Updated README:\n"
    )
    return prompt

def call_openai(prompt):
    client = ChatOpenAI(api_key=os.getenv('OPENAI_API_KEY'), model="gpt-3.5-turbo0125")
    try:
        messages = [
            {"role": "system", "content": "You are an AI trained to help with updateing README files based on commit messages and code file"},
